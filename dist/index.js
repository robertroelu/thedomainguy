...
(() => {
  ...
  var _wrapArray = function _wrapArray2(a, wrapper2, value) {
  ...
  function animateOpenModal(componentEl, instanceEl, duration) {
  ...
  function animateCloseModal(componentEl, duration) {
    const dur = duration / 1e3;
    const tl = gsapWithCSS.timeline();
  ...
  function close(componentElement) {
  ...
  var modal = () => {
    const componentElement = document.querySelector("[global-modal-active]");
    if (!componentElement)
      return;
    ...
    instanceElements.map((instanceElement) => {
      const id = instanceElement.getAttribute("id") || "";
      const index = Number(instanceElement.getAttribute("global-modal-instance"));
      const triggers = Array.from(document.querySelectorAll(`a[href="#${id}"]`));
      if (!triggers.length || !id)
        return;
      const instance = {
        componentElement,
        instanceElement,
    ...
    componentElement.addEventListener("click", (e) => {
      ...
      if (target.getAttribute("global-modal-element") !== "close")
        return;
      close(componentElement);
    });
    closeButtons.forEach((elButton) => {
      const href = elButton.querySelector("a");
  ...
  // node_modules/.pnpm/@finsweet+ts-utils@0.40.0/node_modules/@finsweet/ts-utils/dist/webflow/css.js
  var CMS_CSS_CLASSES = {
    wrapper: "w-dyn-list",
    list: "w-dyn-items",
    item: "w-dyn-item",
  ...
  // node_modules/.pnpm/@finsweet+ts-utils@0.40.0/node_modules/@finsweet/ts-utils/dist/webflow/getCollectionElements.js
  var { wrapper, list, paginationWrapper, paginationNext, paginationPrevious, emptyState, pageCount } = CMS_CSS_CLASSES;
  function getCollectionElements(reference, target, page = document) {
  ...
  // src/nest/loadElement.ts
  var loadElement = async (nestTarget, collectionSlug, collectionItemSlug, collectionId) => {
    const pageSource = `${collectionSlug}/${collectionItemSlug}`;
    const pageData = await fetch(pageSource);
    const pageText = await pageData.text();
    const parser = new DOMParser();
    const doc = parser.parseFromString(pageText, "text/html");
    const selector3 = `#${collectionId}`;
    const wrapper2 = await getCollectionElements(selector3, "wrapper", doc);
    nestTarget.append(wrapper2);
    gsapWithCSS.fromTo(
      nestTarget,
      {
        opacity: 0
  ...
  // src/nest/queryElement.ts
  function queryElements(selector3, parent) {
    return Array.from((parent || document).querySelectorAll(selector3));
  }

  // src/nest/nestElement.ts
  var nestedElement = () => {
    ...
    instances.forEach((instance) => {
      ...
      if (!collectionItemSlug || !collectionSlug || !collectionId)
        return;
